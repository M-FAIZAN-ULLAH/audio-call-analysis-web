...
const EmotionChart = ({
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        <Spin size="large" />
        <div className="mt-4 text-gray-600">Loading analysis data...</div>
      </div>
    );
  }
    ...
    return (
      <div className="p-6">
        <Alert
          ...
          description="Please analyze your audio files first."
          type="info"
          showIcon
        />
      </div>
  ...
  const labels = sortedEmotions.map((emotion) => emotion.name);
  const scores = sortedEmotions.map((emotion) => emotion.score);
  ...
  const getEmotionColor = (emotionName) => {
    ...
    return negativeColors[emotionName] || "#6366F1";
  };
    ...
    datasets: [
      {
        ...
        backgroundColor: labels
          .slice(0, 8)
          .map((label) => getEmotionColor(label)),
        borderWidth: 2,
        borderColor: "#ffffff",
        hoverOffset: 8,
      },
    ...
    datasets: [
      {
        ...
        data: scores.slice(0, 10).map((s) => Math.round(s * 100)),
        backgroundColor: labels
          .slice(0, 10)
          .map((label) => getEmotionColor(label)),
        borderColor: labels
  ...
  // Extract individual files for single call view
